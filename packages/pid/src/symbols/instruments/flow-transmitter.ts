...
/**
 * Flow Transmitter (Strømningstransmitter — FT)
 *
 * ISA 5.1 instrument bubble: circle body x=0..36, y=0..36.
 * Used for air volume flow measurement (duct) or water flow measurement
 * (hydronic circuits). Units: m³/h for water, m³/s or l/s for air.
 *
 * Port x/y from body origin (0,0):
 *   process    = bottom centre: x=18, y=36
 *   signal-out = right centre:  x=36, y=18
 */
export const flowTransmitter: PidSymbolDefinition = {
  id: "sensor-flow",
  ...
  stateRenderer(inputs, ctx) {
    ...
    ctx.set("value", "text", `${inputs.value} m³/h`);
  },
};
