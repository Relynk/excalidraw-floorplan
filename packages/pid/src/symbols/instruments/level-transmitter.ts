...
/**
 * Level Transmitter (Nivåtransmitter — LT)
 *
 * ISA 5.1 instrument bubble: circle body x=0..36, y=0..36.
 * Measures liquid level in tanks, vessels, and expansion vessels.
 * Units: % (0–100 % of tank capacity) or metres.
 *
 * Port x/y from body origin (0,0):
 *   process    = bottom centre: x=18, y=36
 *   signal-out = right centre:  x=36, y=18
 */
export const levelTransmitter: PidSymbolDefinition = {
  id: "sensor-level",
  ...
  ports: [
    {
      ...
      x: 18,
      y: 36,
      acceptsTypes: [PORT_TYPES.PIPE, PORT_TYPES.SIGNAL_4_20MA],
      direction: "in",
    },
    {
      id: "signal-out",
  ...
  stateRenderer(inputs, ctx) {
    ...
    ctx.set("value", "text", `${inputs.value} %`);
  },
};
